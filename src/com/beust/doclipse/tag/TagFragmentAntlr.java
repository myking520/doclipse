...
public class TagFragmentAntlr implements ITagFragment {
	...
	public static void main(String[] argv) {
		// String s = "a=b";
		String[] strings = { "", "@ej", "@ej ", "@ej a", "@ej a ", "@ej a =", "@ej a = ", "@ej a = b", "@ej a = \"b\"",
				"@ej a = b ", "@ej e = \"a b c\" ", "@ej\n  *  e = \"a b c\"\n  * " };

		String[] fragments = { "", "@ej", "", "a", "", "", "", "b", "b", "", "", "" };

		boolean[] completesOnAttribute = { false, false, true, true, false, false, false, false, false, true, true,
				true };

		boolean[] completesOnTagName = { true, true, false, false, false, false, false, false, false, false, false,
				false };

		String[] attributeNames = { "", "", "", "a", "a", "a", "a", "a", "a", "a", "e", "e" };

		for (int i = 0; i < fragments.length; i++) {
			ppp("TEST " + i);
			ITagFragment tf = TagFragmentFactory.newTagFragment(strings[i]);
			assert tf.completesOnTagName() == completesOnTagName[i] : i + " Complete on tagName : found "
					+ tf.completesOnTagName() + " but expected " + completesOnTagName[i];
			assert tf.getFragment().equals(fragments[i]) : i + " Wrong fragment : found '" + tf.getFragment()
					+ "' but expected '" + fragments[i] + "'";
			assert tf.completesOnAttribute() == completesOnAttribute[i] : i + " Complete on attribute : found "
					+ tf.completesOnAttribute() + " but expected " + completesOnAttribute[i];

			String an = tf.getAttributeName() == null ? "" : tf.getAttributeName();
			assert an.equals(attributeNames[i]) : i + " Wrong attributeName : found '" + an + "' but expected '"
					+ attributeNames[i] + "'";
		}
