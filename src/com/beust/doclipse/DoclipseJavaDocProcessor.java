...
/**
 * This class implements the JavaDocCompletionProcessor.
 *
 * @author Cedric Beust, Jun 11, 2004
 * 
 */
public class DoclipseJavaDocProcessor implements IJavadocCompletionProcessor {
  ...
  public IContextInformation[] computeContextInformation(ICompilationUnit cu, int offset) {
    ...
    IContextInformation[] result = new IContextInformation[tags.size()];

    int i = 0;
    for (Iterator it = tags.values().iterator(); it.hasNext(); ) {
      Tag tag = (Tag) it.next();
      result[i] = new ContextInformation("Context", tag.getName());
    }
   ...
   * 
   * @param prefix
   * @return true if the current prefix is a tag with an incomplete
   * attribute (e.g. "@ejbgen:entity ejb-n"
   */
      ...
      int offset, int length, int flags) 
  {
    ...
    m_document = JavaUI.getDocumentProvider().getDocument(editorInput);
//    IDocumentProvider dp = JavaUI.getDocumentProvider();
    ...
    try {
      ...
      if (tf.completesOnTagName()) {
        for (Iterator it = tags.values().iterator(); it.hasNext(); ) {
          ...
          //
          // Only add a proposal if it has the same prefix as what has already been typed
          //
...
//            ppp("SKIPPING " + tag.getName() + " BECAUSE starts:" + starts + " matchTarget:" + matchTarget);
          }
        }
    ...
    IJavaCompletionProposal[] result =
      (IJavaCompletionProposal[]) vResult.toArray(new IJavaCompletionProposal[vResult.size()]);
    return result;
  }
