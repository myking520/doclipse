 ...
 * 
 */
public class ExternalFilesPage extends DoclipsePreferencePage implements IWorkbenchPreferencePage {
	...
	private void refreshExternalTable(Map checkedFiles) {
		m_externalDefinitionFiles = Utils.readDirectory(m_directoryTF.getText(), null);
	...
	private void initializeValues() {
		String dir = null;
		if (isPlugin()) {
			m_directoryTF.setText(doclipseProject.getExternalDirectory());
			m_storedExternalFiles = doclipseProject.getExternalCheckedFiles();
			refreshExternalTable(m_storedExternalFiles);
		} else {
	...
	protected void refreshExternalDirectory() {
		...
		doclipseProject.updateExternalTags(m_externalDefinitionFiles);

		//
		// Save the current values to the IPreferenceStore
		//
		doclipseProject.saveExternalCheckedFiles(fileNames);
		doclipseProject.saveExternalDirectory(m_directoryTF.getText());
	}
